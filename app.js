...
const globalControls = [
  ...
  { key: "domeAccelDegPerSec2", label: "Dome Accel (deg/s²)", min: 0.2, max: 100, step: 0.1 },
  { key: "domeDecelDegPerSec2", label: "Dome Decel (deg/s²)", min: 0.2, max: 100, step: 0.1 },
...
function getEffectiveSlitWidthMm() {
  ...
  return Math.max(0, state.domeRadiusMm * (openingDeg * Math.PI / 180));
}
...
function renderGlobalControls() {
  ...
  for (const control of globalControls) {
    ...
    if (control.key === "domeAzimuthDeg") {
      input.disabled = state.domeFollowsTelescope;
    }
  ...
  for (let i = 0; i < state.telescopes.length; i += 1) {
    ...
    for (const btn of card.querySelectorAll(".move-btn")) {
      btn.addEventListener("click", () => {
        ...
        const target = state.telescopes.find((t) => t.id === id);
        if (!target || !field || !Number.isFinite(step)) return;
...
function buildMountScopeScene3D() {
  ...
  state.telescopes.forEach((scope, idx) => {
    ...
    if (scope.mountType === "EQ") {
      ...
      const opposite = saddle === decA ? decB : decA;
      ...
      lines.push({ a: raHead, b: opposite, widthMm: 32, stroke: "rgba(220,230,246,0.55)" });
    ...
    scene.push({
      ...
      lines,
      circles,
      label: v3(scope.posEW, scope.posNS, scope.posUD)
    });
  });
...
function drawTopView() {
  ...
  const slitHalf = slitOpeningDeg * 0.5;
  const slitStart = domeAzimuthDeg - slitHalf;
  const slitEnd = domeAzimuthDeg + slitHalf;
  const slotDepthPx = 22;
  const innerR = Math.max(8, domePx - slotDepthPx);
  const outerStart = polarToCanvas(cx, cy, domePx, slitStart);
  const outerEnd = polarToCanvas(cx, cy, domePx, slitEnd);
  const innerStart = polarToCanvas(cx, cy, innerR, slitStart);
  const innerEnd = polarToCanvas(cx, cy, innerR, slitEnd);
  const slotPath = [
    `M ${outerStart.x} ${outerStart.y}`,
    `A ${domePx} ${domePx} 0 0 1 ${outerEnd.x} ${outerEnd.y}`,
    `L ${innerEnd.x} ${innerEnd.y}`,
    `A ${innerR} ${innerR} 0 0 0 ${innerStart.x} ${innerStart.y}`,
    "Z"
  ].join(" ");

  const slitInfo = svgEl("text", { x: cx, y: cy - domePx + 22, fill: "#c7d5e8", "font-size": 12, "text-anchor": "middle" });
  svg.append(
    svgEl("path", { d: slotPath, fill: "rgba(3,6,12,0.95)", stroke: "none" }),
    svgEl("line", { x1: outerStart.x, y1: outerStart.y, x2: innerStart.x, y2: innerStart.y, stroke: "rgba(220,230,244,0.45)", "stroke-width": 1 }),
    svgEl("line", { x1: outerEnd.x, y1: outerEnd.y, x2: innerEnd.x, y2: innerEnd.y, stroke: "rgba(220,230,244,0.45)", "stroke-width": 1 }),
    slitInfo
  );
  ...
  slitInfo.textContent = `Slit ${state.slitWidthMm.toFixed(0)} mm (${slitOpeningDeg.toFixed(1)} deg${isCapped ? " capped" : ""}) @ ${domeAzimuthDeg.toFixed(0)} deg ${state.simulateDomeSlew ? `(target ${domeTarget.toFixed(0)} deg, err ${domeErr.toFixed(1)} deg)` : ""}`;
  ...
  scene.forEach((obj) => {
    ...
    const lp = projectTopPt(obj.label, cx, cy, scale);
    svg.append(svgEl("text", { x: lp.x + 10, y: lp.y - 10, fill: obj.color, "font-size": 12 }));
    svg.lastChild.textContent = obj.name;
  });
}
...
function drawSideView() {
  ...
  const slitDeltaSigned = signedDeltaDeg(domeAzimuthDeg, sideRot);
  const slitVisibility = clamp(Math.cos(degToRad(Math.abs(slitDeltaSigned))), 0.22, 1);
  const slitCenterXmm = R * Math.sin(degToRad(slitDeltaSigned));
  const slitTopZ = R + Math.sqrt(Math.max(0, R * R - slitCenterXmm * slitCenterXmm));
  const slitTopY = zToPx(slitTopZ);
  const slitBottomY = wallTop;
  const slitWidthPx = Math.max(22, 2 * R * scale * Math.sin(degToRad(computeSlitOpeningDeg()) * 0.5) * slitVisibility);
  const slitX = xToPx(slitCenterXmm) - slitWidthPx / 2;
  ...
  svg.append(
    svgEl("rect", {
      ...
      rx: Math.max(3, slitWidthPx * 0.08),
    ...
    svgEl("line", {
      ...
      x2: slitX,
      y2: slitBottomY,
      stroke: "rgba(228,238,252,0.45)",
      "stroke-width": 1.2,
      "stroke-linecap": "round"
    }),
    svgEl("line", {
      x1: slitX + slitWidthPx,
      y1: slitTopY,
      x2: slitX + slitWidthPx,
      y2: slitBottomY,
      stroke: "rgba(228,238,252,0.45)",
      "stroke-width": 1.2,
      "stroke-linecap": "round"
  ...
  scene.forEach((obj) => {
    obj.lines.forEach((line) => {
      ...
      svg.append(
        svgEl("line", {
          ...
          y2: b.y,
          stroke: line.stroke,
          "stroke-width": strokePxFromMm(line.widthMm, scale, 1),
          "stroke-linecap": "round"
        })
      );
    });
    ...
    const lp = projectSidePt(obj.label, sideRot, xToPx, zToPx);
    svg.append(svgEl("text", { x: lp.x + 10, y: lp.y - 10, fill: obj.color, "font-size": 12 }));
    svg.lastChild.textContent = obj.name;
  });
...
function describeArc(cx, cy, radius, startDeg, endDeg) {
  const start = polarToCanvas(cx, cy, radius, endDeg);
  const end = polarToCanvas(cx, cy, radius, startDeg);
  const span = Math.abs(endDeg - startDeg);
  const largeArcFlag = span <= 180 ? "0" : "1";
  return `M ${start.x} ${start.y} A ${radius} ${radius} 0 ${largeArcFlag} 0 ${end.x} ${end.y}`;
}

function polarToCanvas(cx, cy, radius, deg) {
...
function buildSlitRibbon3D(radiusMm, azimuthDeg, slitWidthMm, samples = 64) {
  ...
  for (let i = 0; i <= samples; i += 1) {
    const t = i / samples;
    const z = t * (2 * radiusMm);
    const shellR = domeRadiusAtHeight(z, radiusMm);
    if (shellR < 0.5) continue;

    const centerX = dirX * shellR;
    const centerY = dirY * shellR;
    const halfW = Math.min(slitWidthMm * 0.5, shellR * 0.96);

    left.push({
      x: centerX + tanX * halfW,
      y: centerY + tanY * halfW,
      z
    });
    right.push({
      x: centerX - tanX * halfW,
...
function projectTop(pt, cx, cy, scale) {
  return {
    x: cx + pt.x * scale,
    y: cy - pt.y * scale
  };
...
function projectSide(pt, sideRotationDeg, xToPx, zToPx) {
  const rot = degToRad(sideRotationDeg);
  const xh = pt.x * Math.cos(rot) + pt.y * Math.sin(rot);
  return {
    x: xToPx(xh),
    y: zToPx(pt.z)
...
function slitWidthMmToWorld(widthPx, scale) {
  if (scale <= 0) return 0;
  ...
  const domeState = !state.simulateDomeSlew
    ...
    : isSettling
      ...
      : domeErr > 0.05
        ...
        : "on target";
  summary.textContent =
    `Slit width is defined in mm with a cap at ${state.maxSlitOpeningDeg.toFixed(0)} deg. Latitude ${state.latitudeDeg.toFixed(1)} deg. Dome ${state.simulateDomeSlew ? `current ${domeCurrent.toFixed(1)} deg, target ${domeTarget.toFixed(1)} deg, error ${domeErr.toFixed(2)} deg, vel ${runtime.domeAngularVelDegPerSec.toFixed(2)} deg/s (${domeState}).` : `azimuth is ${domeCurrent.toFixed(1)} deg.`} Mode: ${state.domeFollowsTelescope ? "auto-follow" : "manual"}.`;
